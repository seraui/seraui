...
import { AIButton } from './components/site/ai-button';
import {
  ...
  return {
    ...
    h2: ({ children, ...props }: React.ComponentProps<'h2'>) => {
      ...
      return (
        <h2
          id={id}
          className="text-3xl font-semibold mb-4 mt-8 first:mt-0 border-b pb-2"
          {...props}
        >
          {children}
        </h2>
    ...
    blockquote: ({ children, ...props }: React.ComponentProps<'blockquote'>) => {
      return (
        <blockquote
          className="border-l-4 border-blue-500 pl-6 italic my-6 text-gray-600 dark:text-gray-400"
          {...props}
        >
          {children}
        </blockquote>
    ...
    table: ({ children, ...props }: React.ComponentProps<'table'>) => {
      return (
        <div className="my-8 w-full overflow-x-auto">
          <table
            className="w-full border-collapse rounded-lg overflow-hidden shadow-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900"
            {...props}
          >
            {children}
          </table>
    ...
    tr: ({ children, ...props }: React.ComponentProps<'tr'>) => {
      return (
        <tr
          className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-all duration-150 hover:shadow-sm"
          {...props}
        >
          {children}
        </tr>
    ...
    kbd: ({ children, ...props }: React.ComponentProps<'kbd'>) => {
      return (
        <kbd
          className="inline-flex items-center rounded border border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-800 px-2 py-1 text-xs font-mono"
          {...props}
        >
          {children}
        </kbd>
