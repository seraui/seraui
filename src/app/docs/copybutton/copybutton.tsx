...
const NpmIcon = ({ className, active }: IconProps) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" style={{ color: active ? '#CB3837' : 'currentColor' }}>
...
const PnpmIcon = ({ className, active }: IconProps) => (
  <svg stroke="currentColor" fill="currentColor" strokeWidth="0" role="img" viewBox="0 0 24 24" className={className} xmlns="http://www.w3.org/2000/svg" style={{ color: active ? '#F69220' : 'currentColor' }}> {/* Using a distinct color for active pnpm */}
    <title></title>
...
const YarnIcon = ({ className, active }: IconProps) => (
  <svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 496 512" className={className} xmlns="http://www.w3.org/2000/svg" style={{ color: active ? '#2C8EBB' : 'currentColor' }}> {/* Using a distinct color for active yarn */}
...
const BunIcon = ({ className, active }: IconProps) => (
  <svg stroke="currentColor" fill="currentColor" strokeWidth="0" role="img" viewBox="0 0 24 24" className={className} xmlns="http://www.w3.org/2000/svg" style={{ color: active ? '#FBF0DF' : 'currentColor' }}>
    <title></title>
...
const CodeBlock = () => {
  ...
  const commands: Record<'npm' | 'pnpm' | 'yarn' | 'bun', { command: string; package: string; url: string }> = {
    npm: {
      command: 'npx',
  ...
  const tabs: Array<{ id: 'npm' | 'pnpm' | 'yarn' | 'bun'; label: string; icon: React.ComponentType<IconProps> }> = [
      ...
      <div className="w-full rounded-lg bg-zinc-100 dark:bg-zinc-950 relative overflow-hidden border border-zinc-200 dark:border-zinc-800">
        <div className="flex items-center justify-between px-4 py-2 bg-zinc-100 dark:bg-zinc-950 border-b border-zinc-200 dark:border-zinc-800">
          ...
          <div className="flex items-center gap-1">
            <button onClick={handleCopy} className="cursor-pointer z-20 p-2 hover:text-foreground transition-colors relative top-0 right-0 text-zinc-900 dark:text-zinc-50 focus:outline-none" title="Copy code">
        ...
        <div className="text-sm text-left font-mono text-nowrap font-medium bg-white dark:bg-zinc-950 px-4 py-4 overflow-x-auto max-w-full min-w-0">
          ...
          <span className="text-zinc-700 dark:text-zinc-300">{activeCommand.url}</span>
        </div>
      </div>
    </div>
  );
