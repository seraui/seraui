...
const MobileMenu: React.FC<MobileMenuProps> = ({ isOpen, navItems }) => (
    ...
    `}>
        ...
        <div className="pt-4 pb-4 border-t border-gray-200 dark:border-gray-700">
            <div className="px-5">
                <Button variant="outline" className="w-full">Buy Template</Button>
            </div>
        </div>
    </div>
);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          <div className="flex-shrink-0 flex items-center gap-2">
            <LogoIcon />
            <span className="text-xl font-bold text-gray-900 dark:text-white">Qupe</span>
          </div>
  ...
  <section className="relative z-10 text-center py-16 sm:py-24 px-4">
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
        <Button variant="primary" className="w-full sm:w-auto">Get started - for free</Button>
        <Button variant="secondary" className="w-full sm:w-auto">Discover Qupe</Button>
      </div>
    </div>
  </section>
);
...
export default function Hero2() {
  return (
    // This is now the single root component with full width support and dark mode
    // It's a self-contained unit with its own background, shadows, and decorative gradients.
    // The `overflow-hidden` class is important to clip the large gradient blobs.
    <div className="relative w-full overflow-hidden bg-white dark:bg-black">

      {/* Decorative background gradients. They are positioned relative to this container. */}
      {/* `pointer-events-none` ensures they don't interfere with mouse clicks. */}
